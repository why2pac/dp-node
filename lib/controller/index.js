...
module.exports = {
  handler: {
    serverError: (controller, req, res, override, err, statusCode) => {
      if (statusCode !== 404) {
        console.error('-------');
        console.error('[ERROR]');
        console.error('-------');
      ...
      if (override) {
        override(controller, err, statusCode);
      }
      else {
  ...
  delegate: (config, req, res, session, cookie) => {
    var controller = {
      ...
      render: (view, params, opts) => {
        var renderString = await(config.view.render(req, res, view, params, opts));
        controller.finish(renderString);
      },
      renderString: (view, params, opts) => {
        return await(config.view.render(req, res, view, params, opts));
      },
