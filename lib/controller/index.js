...
const symSkipFinishBuffer = Symbol('dp-node.skipFinishBuffer');
class Controller {
  constructor(config, req, res, session, cookie) {
    ...
    const rf = config.view.render;
    this.render = function render(view, params, opts) {
      return Promise.resolve(rf(view, params, opts)).then((s) => this.finish(s));
    };
    this.renderString = function renderString(view, params, opts) {
      return Promise.resolve(rf(view, params, opts));
    };
  }
  ...
  redirect(url, statusCode) {
    if (typeof url === 'number' && typeof statusCode === 'string') {
      const temp = url;
