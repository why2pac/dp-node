...
module.exports = (config) => {
  ...
  var handler = (delegate, req, res) => {
    return async (() => {
      ...
      return await (delegate(ctrl));
    })()
  }

  var delegates = {
    route: (method, path, delegate, middlewares, middlewaresEnd) => {
      var params = [path];
      var fn = undefined;
          ...
          var dp = (callback) => {
            return async(() => {
              ...
              try {
                ...
                controller.handler.serverError(ctrl, req, res, config, new dpError(err, req), 500);
              }
            })();
          };
      ...
      var finalizeMiddleware = (req, res, next) => {
        ...
        else if (res.buffer.body === undefined) {
          res.status(204).send();
        }
        else {
      ...
      // Add action
      params.push((req, res, next) => {
        var done = handler(delegate, req, res, next);
  ...
  if (config.cfg.controller) {
    ...
    var includePreapre = (path, filePath, config) => {
      ...
      methods.forEach((method) => {
        if (typeof routes[method] == 'function') {
          ...
          if (config && config.path && config.path.prefix) {
            if (replaceAll) {
              replaceAll = url.resolve(
                config.path.prefix,
                replaceAll.slice(0, 1) === '/' ? replaceAll.slice(1) : replaceMethod
              );
            }
            else if (replaceMethod) {
              replaceMethod = url.resolve(
                config.path.prefix,
                replaceMethod.slice(0, 1) === '/' ? replaceMethod.slice(1) : replaceMethod
              );
            }
          }
          ...
          routePath = routePath + (suffix ? suffix : '');

          trees.push([method, routePath, routes[method], middleware, middlewareForGlobalEnd]);
        }
      })
    };
    ...
    config.app.use((req, res, next) => {
      var ctrl = controller.delegate(config, req, res, session, cookie);
      controller.handler.serverError(ctrl, req, res, config, new dpError('404 Page Notfound', req), 404);
    });
  }
