...
const autoRegisterMiddlewares = (delegates, config) => {
  ...
  const findConfig = (path, offset) => {
    let item = configs[offset ? 'filter' : 'find'](([p]) => path.startsWith(p));
    ...
    if (item) {
      if ((item[1].path || {}).bubbling && !item[1].path.bubbled) {
        const parentItem = findConfig(path, (offset || 0) + 1);

        if (parentItem && (parentItem.path || {}).prefix) {
          const pre = (parentItem.path || {}).prefix;
          const post = (item[1].path || {}).prefix;

          item[1].path.prefix = `${pre}${pre.endsWith('/') || post.startsWith('/') ? '' : '/'}${post}`;
          item[1].path.bubbled = true;
        }
      }
    }
