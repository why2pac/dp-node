const exportCacheMethods = ['conn', 'get', 'set', 'del', 'expire', 'ttl', 'close'];
const cachers = {};
...
Cache.init = (key, dsn) => {
  ...
  if (!cacheDriver._conn) {
    throw Error('Not Implemented, ._conn.');
  }

  let connLazy;
  const conn = () => {
    if (!connLazy) connLazy = cacheDriver._conn(key, dsn.connection);
    return connLazy;
  };
